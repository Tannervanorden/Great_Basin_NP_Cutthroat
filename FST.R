...
data = read_excel('/Users/tannervanorden/Dropbox/Cutthroat_Research/Cutthroat_Cutt/Updated.xlsx', col_names = TRUE)
columns = names(data)
...
ggplot(data = melted, aes(Var1, Var2, fill = value))+ geom_tile(color = "black", linewidth=.3)+ scale_fill_gradient(low = "white", high = "red", name="FST")  + 
  labs( x = "Location", y = "Location") + theme(plot.title = element_text(hjust = 0.5, size = 15)) + 
